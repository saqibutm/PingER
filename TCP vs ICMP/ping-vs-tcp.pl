...
#my $version="0.2, 1/19/2018, Cottrell";#Fix $opt_p bug, add $port to reports, made $fn unique, comment hostname check, replaced getopts.
my $version="0.4, 1/28/2018, Cottrell";#Fix --help, --count, --port for nping.
# **************************************************************** 
...
Function:
  ...
  and host information  such as name, IP address, contry, region etc.  
Externals:
  Requires nping (requires root/sudo privs), dig
Input:
  It gets information on the PingER hosts from %NODE_DETAILS using:
  wget to get the required file from $url. 
  The required file is saved in tmp with a unique name $fn
  (where the number is based on the process number).
Examples:
 $progname
 $progname --prot 6 --port 22 --count 10 | tee pg-v4-nd.txt
 $progname --help 
Hint:
 To turn the output into a real csv file do something like:
 grep warning pg-v4-db.txt  > pg-v4-nd.csv
Version=$version
";
##############################################################
#Process options
use Getopt::Long;
my $prot=""; my$port='80'; my $help; my $count='10';
...
##############################################################
#Get and execute NODE_DETAILS;
my $wget='wget --no-check-certificate --no-verbose --tries=1 ';
$wget="$wget --quiet --timeout=60 --output-document=$fn '$url'";
my @ans=`$wget`;
our (%NODE_DETAILS, $data_servers);
our (@hep_monitoring_sites, @monitoring_countries,
     @remote_countries,     @monitoring_continents,
     @remote_continents,    @user_defined_groups,);
require "$fn";
#Example of a NODE_DETAILS entry:
#%NODE_DETAILS=(
#    "www.lanl.gov" => [
#    "204.121.60.11",
#    "lanl.gov",
...
foreach my $key (sort keys %NODE_DETAILS) {
   for my $iter (0..49) {
      $n{'lines'}++;
      my $ip=$NODE_DETAILS{$key}[0];
      if($prot eq '6') {
        $ip=dig($key);
        if($ip=~/^warning/) {next;}
      }  
      my $result=get_ping_rtt($ip,$prot,$count);
      if($result=~/^warning/) {$n{'noping'}++; next;}
      $n{'success'}++;
      print "(success=$n{'success'}/noping=$n{'noping'}/$n{'lines'}/$nk)$key($ip,$port), $NODE_DETAILS{$key}[8] "
          . "gave $result\n";
      ...
      if($result1=~/100.00%/)   {$n{'noNping'}++;}
      else { 
        $n{'Npings'}++;
        my $country=$NODE_DETAILS{$key}[5]; $country=~s/\,//g;
        $summary=scalar(localtime()).", summary, $count, $key, $ip, $port, $country, $NODE_DETAILS{$key}[6], $NODE_DETAILS{$key}[8], ping, $result, nping, $result1";
      }
...
sub get_ping_rtt {
  ...
  else {return "warning: no valid ping response for $ip_addr, response:\n@ans";}
}    
#################################################
